...
async def cmd_top(limit: int = 20):
    ...
    async with DatabaseSession() as db:
        ...
        for i, entry in enumerate(entries, 1):
            ...
            voice_name = entry.audio_prompt_path.split('/')[-1] if '/' in entry.audio_prompt_path else entry.audio_prompt_path
            
            print(f"{i:2d}. 🎯 Hits: {entry.hit_count:4d} | ⏱️  {entry.audio_duration_seconds:.1f}s | 🗣️  {voice_name}")
...
async def cmd_evict(count: int = 1000):
    ...
    async with DatabaseSession() as db:
        ...
        evicted = await service.evict_old_entries(
            max_entries=entries_before - actual_count,
...
async def cmd_invalidate(voice_path: str):
    ...
    logger.section(f"INVALIDATE VOICE CACHE")
...
Examples:
  ...
  python scripts/manage_cache.py inspect --key abc123def456
    ...
    # Top command
    top_parser = subparsers.add_parser("top", help="Show most frequently accessed entries")
    top_parser.add_argument("--limit", type=int, default=20, help="Number of entries to show (default: 20)")
    
    # Evict command
    evict_parser = subparsers.add_parser("evict", help="Evict oldest entries")
    evict_parser.add_argument("--count", type=int, default=1000, help="Number of entries to evict (default: 1000)")
    
    # Clear command
    clear_parser = subparsers.add_parser("clear", help="Clear entire cache (WARNING: destructive!)")
    clear_parser.add_argument("--confirm", action="store_true", help="Skip confirmation prompt")
    
    # Invalidate command
    invalidate_parser = subparsers.add_parser("invalidate", help="Invalidate all entries for a specific voice")
    invalidate_parser.add_argument("--voice", required=True, help="Voice S3 path (e.g., 'audio-prompts/voice_123.wav')")
    
    # Inspect command
    inspect_parser = subparsers.add_parser("inspect", help="Inspect a specific cache entry")
