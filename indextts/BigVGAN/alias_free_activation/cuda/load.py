...
def load(force_rebuild=False):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "Please install PyTorch with CUDA support to use the anti_alias_activation_cuda extension."
        )
    try:
    ...
    if platform.system() == "Windows" and "MINGW64" in os.environ.get("MSYSTEM", ""):
        ...
        print(
            "Warning: Detected running in MinGW-w64 (e.g., Git Bash). CUDA extension build is not supported in this environment.",
            file=sys.stderr,
        )
        raise RuntimeError(
