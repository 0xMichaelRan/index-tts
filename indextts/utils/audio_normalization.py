...
def normalize_loudness(
    ...
    if (
        audio is None
        or (hasattr(audio, "numel") and audio.numel() == 0)
        or (isinstance(audio, np.ndarray) and audio.size == 0)
    ):
    ...
    if audio_np.ndim == 1:
        ...
        raise ValueError(
            f"Unsupported audio shape: {original_shape}. Expected 1D or 2D array."
        )
    ...
    if verbose:
        print(">> Converted audio from int16 range to float32 [-1, 1]")
    ...
    try:
        ...
        if original_lufs < -70.0 or np.isnan(original_lufs) or np.isinf(original_lufs):
            ...
            if is_torch:
                ...
                audio_out = (
                    torch.from_numpy(audio_np).to(original_device).to(original_dtype)
                )
            else:
                audio_out = audio_np
        ...
        if verbose:
            print(
                f">> Scaled back to int16 range (max: {np.abs(normalized_audio).max():.0f})"
            )
        ...
        if is_torch:
            ...
            normalized_audio = (
                torch.from_numpy(normalized_audio)
                .to(original_device)
                .to(original_dtype)
            )
        ...
        return _fallback_peak_normalize(
            audio_fallback, target_peak=-3.0, verbose=verbose
        )
...
def _fallback_peak_normalize(
    ...
    if verbose:
        ...
        print(f">> Applied gain: {gain_db:.2f} dB")
        print(
            f">> Scaled back to int16 range (max: {np.abs(normalized).max() if not is_torch else normalized.abs().max():.0f})"
        )
...
def get_audio_lufs(
    audio: Union[np.ndarray, "torch.Tensor"], sample_rate: int
) -> Optional[float]:
    """
    ...
    if not PYLOUDNORM_AVAILABLE:
        warnings.warn(
            "pyloudnorm not available. Cannot measure LUFS.", category=RuntimeWarning
        )
        return None
