...
async def root():
    ...
    return {
        ...
        "engine": "macOS AVFoundation TTS" if platform.system() == "Darwin" else "IndexTTS GPU Inference",
        "endpoints": {
            "/": "API information",
            "/health": "Health check",
...
async def infer(audio_prompt: UploadFile = File(None), text: str = Form(...)):
    ...
    try:
        ...
        if platform.system() == "Darwin":
            ...
            print(f">> GPU inference: Synthesizing text with voice from {original_audio_filename}")
