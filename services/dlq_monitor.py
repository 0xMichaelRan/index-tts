...
class DLQStats:
    ...
    def should_alert(
        self, message_threshold: int, age_threshold_hours: int = 24
    ) -> bool:
...
class DLQMessage:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse DLQ message body as JSON: {self.body[:100]}"
            )
...
class DLQMonitor:
    ...
    def check_and_alert(self) -> Dict[str, DLQStats]:
        ...
        with self._lock:
            ...
            self._stats_history = [
                entry for entry in self._stats_history if entry["timestamp"] > cutoff
            ]
    ...
    def purge_dlq(self, queue_name: str) -> int:
        ...
        try:
            ...
            logger.warning(f"PURGED {message_count} messages from DLQ '{queue_name}'")
            ...
            return [
                {
                    ...
                    "stats": {
                        name: stats.to_dict() for name, stats in entry["stats"].items()
                    },
                }
