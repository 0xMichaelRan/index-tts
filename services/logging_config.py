...
class StructuredFormatter(logging.Formatter):
    ...
    def _get_level_indicator(self, level_name: str) -> str:
        ...
        if level_name == "INFO":
            ...
            if "✓" in str(msg):
                return self.COLORS.get("SUCCESS", "") + "✓" + self.COLORS.get("RESET", "")
            elif "✗" in str(msg):
                return self.COLORS.get("ERROR", "") + "✗" + self.COLORS.get("RESET", "")
            elif "⚠" in str(msg):
                return self.COLORS.get("WARNING", "") + "⚠" + self.COLORS.get("RESET", "")
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if self.compact:
            ...
            formatted = (
                f"{timestamp} - {record.name:30s} - {level:8s} - {original_msg}"
            )
...
def log_circuit_breaker_stats(
    ...
    # Header
    logger.info(f"{'Breaker':20s} {'State':12s} {'Success':10s} {'Failure':10s} {'Rate':10s}")
    logger.info("─" * 70)
