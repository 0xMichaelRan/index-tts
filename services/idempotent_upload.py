...
class IdempotentUploader:
    ...
    def upload_with_retry(
        ...
        existing = self._check_existing_upload(
            job_id, remote_path, bucket_type=bucket_type
        )
        if existing:
            logger.info(
        ...
        file_hash = (
            self._calculate_file_hash(local_path) if verify_integrity else "skipped"
        )
        ...
        while retry_count < self.max_retries:
            try:
                ...
                logger.debug(
                    f"[JOB {job_id}] Metadata (not stored in S3): {metadata.to_dict()}"
                )
